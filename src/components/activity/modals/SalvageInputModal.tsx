...
import { Recycle, Save, X, Plus, Trash2 } from 'lucide-react'
...
export function SalvageInputModal({ open, onOpenChange, onSave, existingItems = [] }: SalvageInputModalProps) {
  const [items, setItems] = useState<SalvageItem[]>(existingItems)
  ...
  const handleParseRawInput = () => {
    ...
    if (parsed.length === 0) {
      toast.error('Could not parse salvage content. Format: "1 x Item Name" per line')
      return
    }
  ...
  const handleItemChange = (index: number, field: keyof SalvageItem, value: string | number) => {
      ...
      <DialogContent className="sm:max-w-[600px] bg-zinc-950 border-zinc-800 text-zinc-100">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-lg font-black uppercase tracking-wider">
            ...
            Add Salvage Contents
          </DialogTitle>
          <DialogDescription className="text-zinc-400 text-sm">
            Paste or manually add items from salvage operations
          </DialogDescription>
        </DialogHeader>
          ...
          {mode === 'raw' ? (
            <div className="space-y-2">
              ...
              <Textarea
                ...
                className="min-h-[150px] bg-zinc-900/50 border-zinc-800 text-zinc-100 font-mono text-sm resize-none focus:border-orange-500"
              />
              <Button 
                onClick={handleParseRawInput}
                className="w-full bg-orange-500 hover:bg-orange-600 text-black font-black uppercase text-xs tracking-wider"
              >
                ...
                {items.map((item, index) => (
                  <div key={index} className="flex gap-2 items-center bg-zinc-900/50 p-2 rounded-lg border border-zinc-800/50">
                    <Input
                      type="number"
                      value={item.quantity}
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleItemChange(index, 'quantity', parseInt(e.target.value) || 0)}
                      className="w-16 h-8 bg-zinc-950 border-zinc-800 text-center font-mono text-sm"
                    />
                    <Input
                      value={item.name}
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleItemChange(index, 'name', e.target.value)}
        ...
        <DialogFooter>
          ...
          <Button 
            ...
            className="bg-orange-500 hover:bg-orange-600 text-black font-black uppercase text-xs tracking-wider"
          >
