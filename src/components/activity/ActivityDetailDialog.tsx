...
export function ActivityDetailDialog({ activity: initialActivity, trigger, open, onOpenChange }: ActivityDetailDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="bg-[#050507] border-white/5 lg:max-w-[900px] md:max-w-[80vw] sm:max-w-[95vw] max-h-[92vh] overflow-hidden flex flex-col p-0 gap-0 shadow-[0_0_80px_rgba(0,0,0,0.8)]">
        
        {/* Header Section */}
        <div className="bg-zinc-900/50 border-b border-white/5 p-4 sm:p-6 lg:p-8 relative">
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
            <DialogHeader className="p-0 text-left flex-1 min-w-0">
              <div className="flex items-center gap-3 mb-3 sm:mb-4">
                <Badge variant="outline" className="px-2 sm:px-3 py-0.5 sm:py-1 text-[9px] sm:text-[10px] uppercase font-black tracking-[0.2em] border-eve-accent/30 text-eve-accent bg-eve-accent/5 rounded-md">
                  {activity.type} OPERATION
                </Badge>
                <div className="h-4 w-[1px] bg-white/10 hidden sm:block" />
                <div className="text-[9px] sm:text-[10px] font-black font-mono text-zinc-600 uppercase tracking-widest truncate">
                  SES_ID: {activity.id.slice(0, 12)}
                </div>
              </div>
              
              <DialogTitle className="text-xl sm:text-3xl lg:text-4xl font-black text-white tracking-tighter flex items-center gap-3 sm:gap-4 flex-wrap">
                <span className="truncate">{data.siteName || (activity.type || 'Activity').toUpperCase()}</span>
                {activity.status === 'completed' && (
                  <Badge className="bg-zinc-800 text-zinc-400 hover:bg-zinc-800 border-none font-black text-[8px] sm:text-[9px] uppercase tracking-widest whitespace-nowrap">
                    Finished
                  </Badge>
                )}
              </DialogTitle>
              
              <DialogDescription className="text-[10px] sm:text-xs text-zinc-500 font-bold uppercase tracking-wider mt-3 sm:mt-4 flex flex-wrap items-center gap-x-4 gap-y-2">
            ...
            <div className="flex flex-row md:flex-col items-center md:items-end justify-between md:justify-start gap-4 flex-shrink-0">
              <div className="flex flex-col items-end gap-1 sm:gap-2">
                <span className="text-[9px] sm:text-[10px] text-zinc-500 font-black tracking-widest uppercase opacity-40">Elapsed Time</span>
                <div className="bg-black/30 border border-white/5 px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg sm:rounded-xl">
                  <span className="text-xl sm:text-2xl font-black font-mono text-eve-accent tracking-tighter tabular-nums">{elapsed}</span>
                </div>
              </div>
              ...
              <div className="flex bg-black/40 p-1 rounded-xl border border-white/5">
                <Button
                  ...
                  className={cn(
                    "h-7 sm:h-8 px-2 sm:px-4 text-[9px] sm:text-[10px] font-black uppercase tracking-widest gap-2 rounded-lg transition-all",
                ...
                >
                  <span className="hidden sm:inline">Reduced View</span>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 sm:p-6 lg:p-8 space-y-6 sm:space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            <div className="bg-zinc-950/60 border border-white/5 rounded-2xl p-4 sm:p-5 lg:p-6 relative overflow-hidden group">
              <div className="relative z-10 space-y-3 sm:space-y-4">
                ...
                <div>
                  <p className="text-xl sm:text-2xl lg:text-3xl font-black text-white font-mono tracking-tighter truncate">{formatISK(totalRevenue)}</p>
            ...
            <div className="bg-zinc-950/60 border border-white/5 rounded-2xl p-4 sm:p-5 lg:p-6 relative overflow-hidden">
               <div className="relative z-10 space-y-3 sm:space-y-4">
                <div className="flex items-center gap-3">
                ...
                <div>
                  {(() => {
                    ...
                    return (
                      <p className="text-xl sm:text-2xl lg:text-3xl font-black text-white font-mono tracking-tighter truncate">
            ...
            {isMiningActivity ? (
              <div className="bg-zinc-950/60 border border-white/5 rounded-2xl p-4 sm:p-5 lg:p-6 relative overflow-hidden sm:col-span-2 lg:col-span-1">
                <div className="relative z-10 space-y-3 sm:space-y-4">
                  <div className="flex items-center gap-3">
