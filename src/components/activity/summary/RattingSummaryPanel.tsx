    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between bg-zinc-950/40 p-4 rounded-2xl border border-white/5">
        ...
        <Button
          ...
          size="sm"
          onClick={onOpenMTU}
          className="h-10 px-4 bg-blue-500/10 border-blue-500/20 text-blue-400 hover:bg-blue-500/20 hover:text-blue-300 text-[10px] font-black uppercase tracking-widest gap-2 w-full sm:w-auto"
        >
        ...
        {characterData.length === 0 ? (
          ...
          characterData.map((group) => (
            <div key={group.charId} className="bg-zinc-950/40 border border-white/5 rounded-2xl overflow-hidden transition-all">
              ...
              <div className={cn(
                "p-5 flex items-center justify-between hover:bg-white/[0.02] transition-colors",
                ...
                <div className="text-right">
                  <p className="text-2xl font-black text-white font-mono tracking-tighter tabular-nums">{formatISK(group.total)}</p>
              ...
              {viewMode === 'detailed' && (
                <div className="bg-black/20 divide-y divide-white/[0.03] max-h-[300px] overflow-y-auto custom-scrollbar">
                  {group.logs.map((log, idx) => (
                    <div key={idx} className="px-6 py-3 flex items-center justify-between group/row">
                      <div className="flex items-center gap-3">
                         ...
                         <div>
                            <p className="text-[10px] font-black uppercase text-zinc-400 tracking-wider">
                              {log.type} <span className="text-zinc-600 mx-2">//</span> {new Date(log.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                            </p>
                         </div>
                      </div>
                      <p className={cn(
