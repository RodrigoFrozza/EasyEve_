...
export async function GET(
  ...
  if (nextauth[0] === 'callback' && nextauth[1] === 'eveonline') {
    const url = new URL(request.url)
    const state = url.searchParams.get('state')
    
    const callbackUrl = new URL('https://login.eveonline.com/v2/oauth/authorize')
    callbackUrl.searchParams.set('client_id', process.env.EVE_CLIENT_ID!)
    callbackUrl.searchParams.set('redirect_uri', `${process.env.NEXTAUTH_URL}/api/auth/callback/eveonline`)
    callbackUrl.searchParams.set('response_type', 'code')
    callbackUrl.searchParams.set('scope', EVE_SCOPES)
    
    const accountCode = request.nextUrl.searchParams.get('accountCode')
    if (accountCode) {
      const customState = { accountCode }
      callbackUrl.searchParams.set('state', JSON.stringify(customState))
    }
